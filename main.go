...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case rowsResultMsg:
		...
		m.status = fmt.Sprintf(
			"Showing rows (page size %d). Press 'b' to go back, 'n'/'p' for next/prev page.",
			m.pageSize,
		)
		m.mode = modeRows
		return m, nil
...
func (m model) viewRows() string {
	...
	s += "\n" + m.status + "\n"
	s += "\nPress 'b' to go back to tables, 'q' or ctrl+c to quit. Use n/p for next/prev page, ←/→ or h/l to scroll horizontally.\n"

	// apply horizontal scroll based on terminal width and offset
	return applyHorizontalScroll(s, m.horizOffset, m.width)
}
